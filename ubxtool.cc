...
std::pair<UBXMessage, struct timeval> getUBXMessage(int fd, double *timeout)
{
  ...
  for (;;)
  {
    ...
    //if (doDEBUG) { cerr<<humanTimeNow()<<" marker now: "<< marker[0]<<" " <<marker[1]<<endl; }
    //std::cout << "Read marker: " << std::hex << static_cast<int>(marker[0]) << " " << static_cast<int>(marker[1]) << std::dec << std::endl;
    //std::cout << "Read markerRaW: " << marker[0] << " " << marker[1] << std::dec << std::endl;

    if (marker[0] == 0xb5 && marker[1] == 0x62)
    { // bingo
      struct timeval tv;
      gettimeofday(&tv, 0);
...
UBXMessage waitForUBX(int fd, int seconds, uint8_t ubxClass, uint8_t ubxType)
{
  ...
  for (;;)
  {
    ...
    else
    {
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Got: " << (int)msg.getClass() << " " << (int)msg.getType() << " while waiting for " << (int)ubxClass << " " << (int)ubxType << endl;
      }
    }
  }
...
bool waitForUBXAckNack(int fd, int seconds, int ubxClass, int ubxType)
{
  ...
  for (;;)
  {
    ...
    if (msg.getClass() != 5 || !(msg.getType() == 0 || msg.getType() == 1))
    {
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Got: " << (int)msg.getClass() << " " << (int)msg.getType() << " while waiting for ack/nack of " << ubxClass << " " << ubxType << endl;
      }
      continue;
    }
    ...
    if (doDEBUG)
    {
      cerr << humanTimeNow() << " Got an " << (msg.getType() ? "ack" : "nack") << " for " << (int)payload[0] << " " << (int)payload[1] << " while waiting for " << ubxClass << " " << ubxType << endl;
    }
...
void enableUBXMessageOnPort(int fd, uint8_t ubxClass, uint8_t ubxType, uint8_t port, uint8_t rate = 1)
{
  for (int n = 0; n < 5; ++n)
  {
    ...
    catch (TimeoutError &te)
    {
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Had " << n << "th timeout in enableUBXMessageOnPort" << endl;
      }
      continue;
    }
  }
...
void readSome(int fd)
{
  ...
  for (int n = 0; n < 5; ++n)
  {
    ...
    try
    {
      ...
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Read some init: " << (int)msg.getClass() << " " << (int)msg.getType() << endl;
      }
      if (msg.getClass() == 0x4)
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " " << string((char *)msg.getPayload().c_str(), msg.getPayload().size()) << endl;
        }
    }
    catch (TimeoutError &te)
    {
...
void doTermios(int fd, bool doRTSCTS)
{
  ...
  if (doRTSCTS)
  {
    if (doDEBUG)
    {
      cerr << humanTimeNow() << " will enable RTSCTS" << endl;
    }
  ...
  //std::cout<<"baudrate"<<g_baudval<<std::endl;
  ...
  if (doDEBUG)
  {
    cerr << humanTimeNow() << " initFD - tty set" << endl;
  }
}
...
int initFD(const char *fname, bool doRTSCTS)
{
  ...
  if (doDEBUG)
  {
    cerr << humanTimeNow() << " initFD()" << endl;
  }
  if (!isPresent(fname))
  {
    if (doDEBUG)
    {
      cerr << humanTimeNow() << " initFD - " << fname << " - not present" << endl;
    }
  ...
  if (string(fname) != "stdin" && string(fname) != "/dev/stdin" && isCharDevice(fname))
  {
    if (doDEBUG)
    {
      cerr << humanTimeNow() << " initFD - open(" << fname << ")" << endl;
    }
    //std::cout<<"readandwrite"<<std::endl;
    ...
    if (doDEBUG)
    {
      cerr << humanTimeNow() << " initFD - open successful" << endl;
    }
  ...
  else
  {
    ...
    if (doDEBUG)
    {
      cerr << humanTimeNow() << " initFD - open(" << fname << ") - from file" << endl;
    }
...
int main(int argc, char **argv)
{
  ...
  if (!g_fromFile)
  {
    ...
    if (doInit)
    {
      ...
      if (doReset)
      {
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Sending a soft reset" << endl;
        }
        ...
        for (int n = 0; n < 20; ++n)
        {
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Waiting for device to come back" << endl;
          }
          try
          {
          ...
          catch (...)
          {
            if (doDEBUG)
            {
              cerr << humanTimeNow() << " Not yet back" << endl;
            }
            usleep(400000);
            continue;
          }
      ...
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Sending version query" << endl;
        cerr << fd<< endl;
      }
      UBXMessage um1 = sendAndWaitForUBX(fd, 5, msg, 0x0a, 0x04); // ask for version
      ...
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Sending serial number query" << endl;
      }
      ...
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Sending GNSS query" << endl;
      }
      ...
      try
      {
        if (waitForUBXAckNack(fd, 2, 0x06, 0x3e))
        {
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Got ACK for our poll of GNSS settings" << endl;
          }
        }
      }
      catch (...)
      ...
      if (!version9)
      {
        ...
        { // GNSS setting
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Got ack on GNSS setting" << endl;
          }
        }
        else
        {
      ...
      { // UBX-CFG-VALSET
        //                                 vers  ram   res   res
        msg = buildUbxMessage(0x06, 0x8a, {0x00, 0x01, 0x00, 0x00, 0x1f, 0x00, 0x31, 0x10, doGPS, //
                                           0x01, 0x00, 0x31, 0x10, doGPS, 0x03, 0x00, 0x31, 0x10, doGPS,

                                           0x21, 0x00, 0x31, 0x10, doGalileo, 0x07, 0x00, 0x31, 0x10, doGalileo, 0x0a, 0x00, 0x31, 0x10, doGalileo,

                                           0x22, 0x00, 0x31, 0x10, doBeidou, 0x0d, 0x00, 0x31, 0x10, doBeidou, 0x0e, 0x00, 0x31, 0x10, doBeidou,

                                           0x25, 0x00, 0x31, 0x10, doGlonass, 0x18, 0x00, 0x31, 0x10, doGlonass, 0x1a, 0x00, 0x31, 0x10, doGlonass});

        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Sending F9P GNSS setting, GPS: " << doGPS << ", Galileo: " << doGalileo << ", BeiDou: " << doBeidou << ", GLONASS: " << doGlonass << ", SBAS: " << doSBAS << endl;
        }

        if (sendAndWaitForUBXAckNack(fd, 2, msg, 0x06, 0x8a))
        { // GNSS setting, F9 stylee
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Got ack on F9P GNSS setting" << endl;
          }
        }
        else
        {
        ...
        */
        ...
        msg = buildUbxMessage(0x06, 0x8a, {0x00, 0x01, 0x00, 0x00, 0x07, 0x00, 0x91, 0x20, 1, 0x32, 0x02, 0x91, 0x20, 1});
        if (sendAndWaitForUBXAckNack(fd, 2, msg, 0x06, 0x8a))
        { // msg cfg F9P
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Got ack on F9P UART1 setting" << endl;
          }
        }
        else
        {
      ...
      if (m8t)
      {
        ...
        try
        {
          if (waitForUBXAckNack(fd, 2, 0x06, 0x3d))
          {
            if (doDEBUG)
            {
              cerr << humanTimeNow() << " Got ACK for our poll of TMODE2" << endl;
            }
          }
        }
        catch (...)
      ...
      if (doSurveyReset)
      {
        ...
        if (sendAndWaitForUBXAckNack(fd, 2, msg, 0x06, cmd))
        {
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Got ack on survey-reset" << endl;
          }
        }
        else
        {
      ...
      if (surveyMinSeconds || surveyMinCM)
      {
        ...
        {
          cmd = 0x8a;
          msg = buildUbxMessage(0x06, cmd, {0x00, 0x01, 0x00, 0x00, 0x01, 0x00, 0x03, 0x20, 1, // survey in mode
                                            // min survey time:
                                            0x10, 0x00, 0x03, 0x40, ptrSeconds[0], ptrSeconds[1], ptrSeconds[2], ptrSeconds[3], 0x11, 0x00, 0x03, 0x40, ptrCent[0], ptrCent[1], ptrCent[2], ptrCent[3]});
        }
        else
        {
        ...
        if (sendAndWaitForUBXAckNack(fd, 2, msg, 0x06, cmd))
        {
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Got ack on survey-in" << endl;
          }
        }
        else
        {
      ...
      if (doSBAS)
      {
        ...
        if (sendAndWaitForUBXAckNack(fd, 10, msg, 0x06, 0x16))
        { // enable SBAS
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Got ack on SBAS setting" << endl;
          }
        }
        else
        {
      ...
      if (!doKeepNMEA || (newbaudrate && newbaudrate != baudrate))
      {
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Changing port settings (newbaudrate=" << newbaudrate << ", baudrate=" << baudrate << ", keepNMEA=" << doKeepNMEA << ")" << endl;
        }
        ...
        if (ubxport == 1 || ubxport == 2)
        {
          ...
          msg = buildUbxMessage(0x06, 0x00, {(unsigned char)(ubxport), 0x00, 0x00, 0x00, 0xC0 /* 8 bit */, 0x08 /* No parity */, 0x00, 0x00, (unsigned char)((actbaud) & 0xFF), (unsigned char)((actbaud >> 8) & 0xFF), (unsigned char)((actbaud >> 16) & 0xFF), (unsigned char)((actbaud >> 24) & 0xFF),
        ...
        { //   port                 res   tx-ready  res  res  res  res  res  res  res res   in   in     out  out  res   res  res res
        ...
        {
          if (sendAndWaitForUBXAckNack(fd, 10, msg, 0x06, 0x00))
          { // disable NMEA
            if (doDEBUG)
            {
              cerr << humanTimeNow() << " ACK for new port/protocol settings" << endl;
            }
          }
          else
          {
            if (doDEBUG)
            {
              cerr << humanTimeNow() << " Got NACK for port/protocol settings" << endl;
            }
          }
        }
      }
      /*
      //UBX-CFG-PRT not be available on max 10s
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Polling port settings" << endl;
      } // UBX-CFG-PRT, 0x03 == USB
      ...
      try
      {
        if (waitForUBXAckNack(fd, 2, 0x06, 0x00))
        {
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Got ACK for our poll of port protocol settings" << endl;
          }
        }
      }
      catch (...)
      ...
      */
      ...
      if (mods.find("NEO-M8P") == string::npos)
      {
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Enabling UBX-RXM-RLM" << endl;
        }                                                // SAR
      ...
      */
      ...
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Enabling UBX-MON-HW" << endl;
      }
      ...
      */
      ...
      if (version9)
      {
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Enabling UBX-NAV-SVIN" << endl;
        } // Survey-in results
      ...
      else if (m8t)
      {
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Enabling UBX-TIM-SVIN" << endl;
        } // Survey-in results
      ...
      if (mods.find("NEO-M9N") == string::npos)
      {
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Enabling UBX-RXM-RAWX" << endl;
        }                                                   // RF doppler
      ...
      */
      ...
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Enabling UBX-NAV-CLOCK" << endl;
      }                                                    // clock details
      ...
      if (1)
      {
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Enabling/disabling UBX-NAV-TIMEGPS" << endl;
        }                                                                // GPS time solution
        enableUBXMessageOnPort(fd, 0x01, 0x20, ubxport, doGPS ? 16 : 0); // UBX-NAV-TIMEGPS

        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Enabling/disabling " << doGlonass << " UBX-NAV-TIMEGLO" << endl;
        }                                                                    // GLONASS time solution
        enableUBXMessageOnPort(fd, 0x01, 0x23, ubxport, doGlonass ? 16 : 0); // UBX-NAV-TIMEGLO

        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Enabling/disabling UBX-NAV-TIMEBDS" << endl;
        ...
        if (mods.find("NEO-M8P") == string::npos)
        {
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Enabling/disabling UBX-NAV-TIMEGAL" << endl;
      ...
      if (!version9 && !m8t && !fuzzPositionMeters)
      {
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Enabling debugging data" << endl;
        } // RF doppler
      ...
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Enabling UBX-RXM-SFRBX" << endl;
      }                                                // raw navigation frames
      enableUBXMessageOnPort(fd, 0x02, 0x13, ubxport); // SFRBX

      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Enabling UBX-NAV-POSECEF" << endl;
      }                                                   // position
      ...
      if (version9)
      {
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Enabling UBX-NAV-SIG" << endl;
      ...
      else
      {
        if (doDEBUG)
        {
          cerr << humanTimeNow() << " Enabling UBX-NAV-SAT" << endl;
        }                                                   // satellite reception details
      ...
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Enabling UBX-NAV-PVT" << endl;
      }                                                   // position, velocity, time fix
  ...
  */
  ...
  tstate.doGPS = doGPS;
  tstate.doGalileo = doGalileo;
  tstate.doGlonass = doGlonass;
  ...
  for (;;)
  {
    try
    {
      ...
      if (msg.getClass() == 0x01 && msg.getType() == 0x07)
      { // UBX-NAV-PVT
        struct PVT
        {
          uint32_t itow;
        ...
        if (!g_gnssutc.tv_sec)
        {
          ...
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Got initial timestamp: " << humanTime(satt) << endl;
          }
        }
      ...
      if (!doFakeFix)
      {
        if (!g_gnssutc.tv_sec)
        {
          ...
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Ignoring message with class " << (int)msg.getClass() << " and type " << (int)msg.getType() << ": have not yet received a timestamp" << endl;
          }
          continue;
        }
      }
      ...
      if (msg.getClass() == 0x27 && msg.getType() == 0x03)
      { // serial
        serialno = format_serial(payload);
        if (doDEBUG)
          cerr << humanTimeNow() << " Serial number from stream " << serialno << endl;
      }
      else if (msg.getClass() == 0x02 && msg.getType() == 0x15)
      { // RAWX, the doppler stuff
        ...
        for (int n = 0; n < payload[11]; ++n)
        {
          ...
          { // version 8 defaults galileo to E1B
            sigid = 1;
          }
      ...
      else if (msg.getClass() == 0x01 && msg.getType() == 0x01)
      { // POSECF
        struct pos
        {
          uint32_t iTOW;
      ...
      else if (msg.getClass() == 2 && msg.getType() == 0x13)
      { // SFRBX
        ...
        try
        {
          ...
          { // GALILEO
            ...
            uint32_t satTOW;
            int msgTOW{0};
            if (getTOWFromInav(inav, &satTOW, &g_galwn))
            { // 0, 6, 5
              ...
              if (sigid == 5)
              {
                ...
                {                      // dummy
                  if (id.second != 20) // known broken XXX
                    if (doDEBUG)
                    {
                      cerr << humanTimeNow() << " galileo E" << id.second << " what kind of wtype is this: " << wtype << endl;
                    }
                  continue;
                }
              }
          ...
          else if (id.first == 3)
          {
            ...
            if (bm.wn < 0)
            {
              if (doDEBUG)
              {
                cerr << humanTimeNow() << " BeiDou C" << id.second << " WN not yet known, not yet emitting message" << endl;
              }
              continue;
            }
            NavMonMessage nmm;
        ...
        catch (CRCMismatch &cm)
        {
          if (doDEBUG)
          {
            cerr << humanTimeNow() << " Had CRC mismatch!" << endl;
          }
        }
      }
      else if (msg.getClass() == 1 && msg.getType() == 0x35)
      {               // UBX-NAV-SAT
      ...
      else if (msg.getClass() == 1 && msg.getType() == 0x43)
      { // UBX-NAV-SIG
        for (unsigned int n = 0; n < payload[5]; ++n)
        {
          ...
          if (version9)
          { // we only use this on version9 right now tho
      ...
      else if (msg.getClass() == 1 && msg.getType() == 0x30)
      { // UBX-NAV-SVINFO
      }
      else if (msg.getClass() == 1 && msg.getType() == 0x22)
      { // UBX-NAV-CLOCK
        struct NavClock
        {
          uint32_t iTowMS;
      ...
      else if (msg.getClass() == 0x02 && msg.getType() == 0x14)
      { // UBX-RXM-MEASX
      ...
      else if (msg.getClass() == 0x02 && msg.getType() == 0x59)
      { // UBX-RXM-RLM
      ...
      else if (msg.getClass() == 0x01 && msg.getType() == 0x3B)
      { // UBX-NAV-SVIN
        struct NavSin
        {
          uint8_t ver;
      ...
      else if (msg.getClass() == 0x0d && msg.getType() == 0x04)
      { // UBX-TIM-SVIN
        struct TimSin
        {
          uint32_t dur;
      ...
      else if (msg.getClass() == 0x0a && msg.getType() == 0x09)
      { // UBX-MON-HW
        struct MonHW
        {
      ...
      else if (msg.getClass() == 0x01 && msg.getType() == 0x25)
      { // UBX-NAV-TIMEGAL
      ...
      else if (msg.getClass() == 0x01 && msg.getType() == 0x24)
      { // UBX-NAV-TIMEBDS
      ...
      else if (msg.getClass() == 0x01 && msg.getType() == 0x23)
      { // UBX-NAV-TIMEGLO
      ...
      else if (msg.getClass() == 0x01 && msg.getType() == 0x20)
      { // UBX-NAV-TIMEGPS
      ...
      {
        cerr << humanTimeNow() << " Unknown UBX message of class " << (int)msg.getClass() << " and type " << (int)msg.getType() << " of " << payload.size() << " bytes" << endl;
      }
    ...
    catch (UBXMessage::BadChecksum &e)
    {
      if (doDEBUG)
      {
        cerr << humanTimeNow() << " Bad UBX checksum, skipping message" << endl;
      }
    }
